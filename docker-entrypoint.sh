...
if [ $? -ne 0 ];then
    ...
    orange store -t mysql -d ${ORANGE_DATABASE} -hh ${ORANGE_HOST} -pp ${ORANGE_PORT} -p ${ORANGE_PWD} -u ${ORANGE_USER} -o=init -f=/usr/local/orange/install/orange-v${ORANGE_VERSION}.sql
fi
